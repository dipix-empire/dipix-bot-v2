...
export default new Module(
  ...
  async (app: App, appBusModule: AppBusModuleComponent, logger: Logger) => {
    ...
    let ModalActionRowBiography = new ModalBuilder()
      .setCustomId(`join:modal_2`)
    ...
    return [
      new DiscordEvent("interactionCreate", async (interaction: Interaction) => {
        ...
        try {
          if (await app.prisma.user.findFirst({ where: { discord: interaction.user.id } }) != null) return interaction.reply({ content: '❌ Вы уже игрок. (How Did We Get Here?)', ephemeral: true })
      ...
      new DiscordEvent("interactionCreate", async (interaction: Interaction) => {
        ...
        try {
          ...
          // вот это сейчас надо перенести в отдельный ивент
          // const collector = interaction.channel?.createMessageComponentCollector({ time: 30 * 60 * 1000 });
      ...
      new DiscordEvent("interactionCreate", async (interaction: Interaction) => {
        ...
        try {
          ...
          if (interaction.customId.startsWith(`join:user:send:`)) {
            ...
            Embed(Text_1, Text_2, TextInput).setTitle('Ваша заявка на присоединение')
            await interaction.message.edit({ embeds: [Embed(Text_1, Text_2, TextInput)], components: [ButtonActionRowUser(request.id, true, true)] })
            ...
            interaction.editReply({ embeds: [Embed(Text_1, Text_2, TextInput)], components: [ButtonActionRowUser(request.id, false, false)] })
          }
          else if (interaction.customId.startsWith(`join:user:biography:`)) {
            //int.reply({content: `⛔ В разработке!`, ephemeral: true})
            await interaction.showModal(ModalActionRowBiography);
